...
#include "dRotationTensord3.h"
#include "libmesh/libmesh.h"

dRotationTensord3::dRotationTensord3(Axis axis, Real angle) { update3(axis, angle); }

dRotationTensord3::dRotationTensord3(const RealVectorValue & euler_angles) { update3(euler_angles); }

void
dRotationTensord3::update3(Axis axis, Real angle)
{
  zero();
...
void
dRotationTensord3::update3(const RealVectorValue & euler_angles)
{
  ...
  _coords[0] = c1 * dc3 - c2 * s1 * ds3;  // R11d3
  _coords[3] = -c1 * ds3 - c2 * dc3 * s1; // R12d3
  _coords[6] = 0;                         // R13d3

  _coords[1] = dc3 * s1 + c1 * c2 * ds3; // R21d3
  _coords[4] = c1 * c2 * dc3 - s1 * ds3; // R22d3
  _coords[7] = 0;                        // R23d3

  _coords[2] = s2 * ds3; // R31d3
  _coords[5] = dc3 * s2; // R32d3
  _coords[8] = 0;        // R33d3
}
